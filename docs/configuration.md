...
| `/api/config/ui` | `GET` | yes | Server-provided deploy config the SPA needs to build outbound links: `{"temporalUiBaseUrl": "...", "temporalNamespace": "..."}`. `temporalUiBaseUrl` is empty (and the run overview's Temporal-workflow link omitted) unless `TEMPORAL_UI_URL` is set; `temporalNamespace` is the namespace resolved from the Temporal client config profile. Carries no per-run or sensitive data. |

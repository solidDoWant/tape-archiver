...
| `observability.logsFieldPrefix` | `""` | `VICTORIALOGS_FIELD_PREFIX` — field-name prefix for the worker's `slog` fields, for collectors that nest them under a merge key instead of shipping them top-level (a fluentbit/fluentd `kubernetes` filter with `Merge_Log_Key <key>`). Value is the merge key plus a trailing dot, e.g. `log_fields.`. Empty (default) = the top-level-field shape. See [Live run logs](configuration.md#get-apirunsrunidlogs-log-panel-issue-274). |

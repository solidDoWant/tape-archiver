...
dockerTools.streamLayeredImage {
  ...
  config = {
    ...
    # Container health via the worker's own `healthcheck` self-probe: this image
    # ships no curl/wget, so the binary probes its local /readyz endpoint and
    # exits 0/non-zero (SPEC §4.1). Health reflects readiness — a worker that has
    # lost its Temporal connection reports unhealthy. Durations are in nanoseconds
    # per the OCI image config schema.
    Healthcheck = {
      Test = [
        "CMD"
        "/bin/worker"
